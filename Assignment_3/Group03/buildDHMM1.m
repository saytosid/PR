function lambda = buildDHMM1(D,N,M)
...
THRESHOLD = 0.00000000000000000000000000000000000000001;
%P(D|lambda)
P = zeros(length(D),1);
prevP = ones(length(D),1);
for i = 1:length(D)
	if(P_DHMM(D{i},lambda) > 0 && P_DHMM(D{i},lambda) < 1)
		prevP(i,1) = P_DHMM(D{i},lambda);
	end
end
while(true)
